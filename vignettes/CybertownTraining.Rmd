...
remotes::install_github("USEPA/rExpertQuery", ref = "cybertown_update", dependencies = TRUE, force = TRUE)
...
```{r add.pkgs, message=FALSE, results='hide'}
...
![A screenshot of the web page to sign up for your unique Expert Query
API Key](images/apikey.png){width="3.25in" height="2.32in"}
...
open to dipslay the profile
options.](images/EQprofiles.png){width="3in"}
...
page](images/nat.downloads.png){width="3in"}
...
2.  **Which waters are impaired for SELENIUM nationally? Which states
    ...
    ```{r sel.map, fig.cap="Fig 1. States with selenium listed as a cause of impairment for any waters are shown in blue.", fig.alt = "Fig 1. A map of the United States, including Alaska and Hawaii. Any state with selenium listed as a cause of impairment for any waters is filled in blue.", fig.width = 3}
...
5.  **Which states have included any information about
    ...
    There are `r nrow(orgs.mls)` with monitoringLocationId data in
    ATTAINS. Of these states, `r nrow(links.mls)` also contain
    monitoringLocationDataLinks. We can create a data table to display
    this information and apply some conditional formatting to highlight
    the states with monitoringLocationIdentifiers in blue and those with
...
2.  **What waters in Montana are impaired due to Zinc? How many
    ...
    ```{r MT.plot, fig.with = 2.5, fig.height = 2, fig.cap="Fig 2. Zinc impairments by useName for Montana's latest assessment cycle.", fig.alt = "Fig 2. A bar graph showing then number of Zinc impairments by useName for Montana's latest cycle."}
...
3.  **Which waters are new to category 5 in the 2024 cycle for Illinois?
    ...
    ```{r cat5.viz, fig.height = 2, fig.cap="Fig 3. New category 5 waters in Illinois in the 2024 cycle, summarized by their 2022 categories.", fig.alt = "Fig 3. A pie chart showing new category 5 waters in Illinois in the 2024, cycle summarized by their 2022 categories."}
    # create plotly plot
    plotly::plot_ly() %>%
...
1.  **What are all the TMDLs in HI? How many are there (counted as
    ...
    ```{r hi.tmdl.plot, fig.cap="Fig 4. Number of TMDLs per pollutantGroup in Hawaii.", fig.alt="Fig 4. a bar graph showing Number of TMDLs per pollutantGroup in Hawaii."}
...
1.  **What are all the actions documents that contain the keyword
    ...
    so many results, we'll take a look at the highest ranking 50 matches
    in a [DT](https://rstudio.github.io/DT/) data table to understand
...
2.  **How many actions documents that contain the keyword "nutrient" are
    ...
    ```{r nutrients.plot, fig.height=2, fig.cap="Fig 5. Number of documents containing the keyword "nutrient" by EPA Region.", fig.alt="Fig 5. A bar plot of the number of documents containing the keyword "nutrient" by EPA Region."}
    # create bar plot
...
2.  **Are there active assessmentUnits without any listed
    ...
    There are `r length(n_no.ml.ak)` active assessmentUnits in Alaska
...
2.  **How large of a catchment area corresponds to ""IL_N-99"?**
    ...
    ```{r catch.map, fig.cap="Fig. 6 Map of assessment unit IL_N-99 (blue) and its associated catchments (orange)."}
    # create leaflet map
    leaflet::leaflet() %>%
