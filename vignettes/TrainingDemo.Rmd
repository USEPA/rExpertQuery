...
1.  How many stream miles in Kansas are impaired for primary contact
    ...
    miles in Kansas impaired for primary contract reacreation use.
...
2.  **How many TMDL projects were approved so far in FY 2024 in R10?**

```{r}
tmdl.proj <- rExpertQuery::EQ_TMDLs(region = 10,
                                    tmdl_date_start = "2024-10-01",
                                    tmdl_date_end = "2025-09-30",
                                    api_key = testkey)
```
...
```{r HI.actions, echo = TRUE, results = FALSE, message = FALSE, warning = FALSE} # query Actions from Hawaii that are included in Measures HI.meas <- rExpertQuery::EQ_Actions(api_key = testkey, statecode = "HI", in_meas = "Yes")}
```
...
```{r HI.actions.dt, include = TRUE} # create data table DT::datatable(HI.meas %>%   rExpertQuery::EQ_FormatPlanLinks(),   escape = FALSE)}
```
...
```{r hi.actions.fig}  HI.meas.sum <- HI.meas %>%   dplyr::select(parameterGroup, actionId) %>%   dplyr::group_by(parameterGroup) %>%   dplyr::summarize(count = dplyr::n_distinct(actionId))  plotly::plot_ly(HI.meas.sum, labels = ~parameterGroup, values = ~count, type = 'pie')}
```
...
```{r mo.data, results = TRUE, message = FALSE, warning = FALSE} # query Actions for Missouri Actions with action agency "EPA" and parameter group "PATHOGENS" MO.state.act <- rExpertQuery::EQ_Actions(   api_key = testkey, statecode = "MO",   act_agency = "State",   param_group = "PATHOGENS",   comp_date_start = "2000-01-01",   comp_date_end = "2020-12-31" )  DT::datatable(MO.state.act %>%   rExpertQuery::EQ_FormatPlanLinks(),    options = list(pageLength = 10, scrollX = TRUE), escape = FALSE)}
```
...
```{r mo.data.vis} MO.state.sum <- MO.state.act %>%   dplyr::select(parameter, actionId, actionType) %>%   dplyr::group_by(parameter, actionType) %>%   dplyr::summarize(count = dplyr::n_distinct(actionId))  plotly::plot_ly(data =MO.state.sum,           x = ~actionType, y = ~count, type = 'bar', color = ~parameter) %>%     plotly::layout(barmode = 'stack')}
```
