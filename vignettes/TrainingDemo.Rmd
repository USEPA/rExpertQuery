...
2.  **Which waters are impaired for SELENIUM nationally? Which states
    ...
    sel.map.data <- as.data.frame(state.names) %>%
      dplyr::mutate(SeleniumImpairment = factor(ifelse(state %in% state.names.sel$state,
                                                "yes", "no")))
...
5.  **Which states have included any information about
    monitoringLocations in ATTAINS?**
...
2.  **What waters in Montana are impaired for Aquatic Life due to
    Zinc?**
...
2.  **How many actions (by actionId) were established in R3 in FY
    2024?**
...
act.fy.24.subset <- act.fy.24 %>%
  dplyr::group_by(state, organizationId, organizationName, actionId) %>%
