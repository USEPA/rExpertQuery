...
1.  **What are all the TMDLs in HI? How many are there (counted as
    ...
    There `r formatC(nrow(HI.tmdls), big.mark = ",")` rows can be
    reviewed in the data frame. Keep in mind that if we want to count
    one TMDL as one unique combination of assessmentUnitIdentifier,
    pollutant and actionId, we can't simply count the number of rows as
    this would lead to double counting as each
...
2.  **How many TMDL projects (by actionId) were approved so far in FY
    ...
    [DT](https://rstudio.github.io/DT/) datatable, we will have to take
    a few more steps. We select the columns. The columns,
...
2.  **How many actions (by actionId) were established in R3 in FY 2024?
    ...
    by actionId established in R3 in FY 2024. Next we can create data
    table with counts for each actionType.
...
2.  **Are there active assessmentUnits without any listed
    ...
    There are `r nrow(no.ml.ak)` active assessmentUnits in Alaska
...
1.  **Which states in Region 6 have listed Sources in their latest
    ...
    We will start be querying for EQ_Source for region 6. Then use
    [dplyr](https://dplyr.tidyverse.org/) and
